...
class SquareLattice:
    def __init__(self, L:int, t_hop:float, E_amplitude: Union[float, Callable[[float], float]], E_dir:np.ndarray, h: float, steps: int, initial_occupation: float=None):
        ...
        self.t_hop = t_hop
        self.t = 0 # time
        self.cell_path = np.array([(0, 1), (1, self.L+1), (self.L+1, self.L), (self.L, 0)]) # path around a cell for curl calculation
    ...
    def animate_current_density(self, filename: str, sample_every:int=1, **save_format_kwargs) -> None:
        fig, ax = plt.subplots(figsize=(2*self.L, 2*self.L))
        ...
        animation.save(filename, writer="imagemagick", **save_format_kwargs)
    ...
    def plot_current_density(self, state_index: int, ax: matplotlib.axes = None):
        ...
        for (i, j), (x, y) in positions.items():
            ...
            if idx in curl.keys():
                curl_val = curl[idx]
                curl_circle = plt.Circle((x+0.5, y-0.5), 0.2, facecolor="none", zorder=3, edgecolor="black")

                ax.text(x+0.5, y-0.5, f"{curl_val:.2f}", color='black', ha='center', va='center', fontsize=10)
    ...
    def curl(self, J: np.ndarray) -> np.ndarray:
        cell_width = (self.cell_path.flatten() % self.L).max()
        cell_height = (self.cell_path.flatten() // self.L).max()
        curl = dict()
        #curl_row_length = self.L-cell_width

        for i in range(0, self.L-cell_height, cell_height):
            for j in range(0, self.L-cell_width, cell_width):
                site_index = self.L*i + j
...
class BrickwallLattice(SquareLattice):
    def __init__(self, *args, **kwargs):
        ...
        self.cell_path = np.array([(0, 1), (1, 2), (2, self.L+2), (self.L+2, self.L+1), (self.L+1, self.L), (self.L, 0)])
