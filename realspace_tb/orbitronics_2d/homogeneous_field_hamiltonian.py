...
class HomogeneousFieldAmplitude(ABC):
    ...
    def integrate_to_time(self, time: "float | B.Array") -> "float | B.Array":
        ...
        """
        raise NotImplementedError("Integration not implemented for this field amplitude.")
...
class RampedACFieldAmplitude(HomogeneousFieldAmplitude):
    ...
    def integrate_to_time(self, t: "float | B.Array") -> "float | B.Array":
        ...
        if xp.isscalar(t):
            ...
            s = t if t < T else T
            if t > 0.0 and T > 0.0:
                integral += (T**2 * w**2 - pi*T*w) * xp.cos((s*T*w + pi*s) / T) + (T**2 * w**2 + pi*T*w) * xp.cos((s*T*w - pi*s) / T) + (2*pi**2 -2*T**2*w**2)*xp.cos(w*s) -2*pi**2
                integral /= 4 * w * (T**2 * w**2 - pi**2)
            if t > T:
                integral += (xp.cos(w*T) - xp.cos(w*t)) / w
            return self.E0 * integral

        raise NotImplementedError("Integration of array time inputs not implemented yet.")
...
class LinearFieldHamiltonianPeierls(Hamiltonian):
    ...
    def __init__(
        ...
        # for Peierls substitution, we need a phase shift matrix with elements theta_kl = (r_k - r_l) . E0
        size = geometry.Lx * geometry.Ly
    ...
    def at_time(self, t: float) -> B.SparseArray:
        ...
        phase_factors = B.xp().exp(-1j * self.theta_matrix.data * self.field_amplitude.integrate_to_time(t))
