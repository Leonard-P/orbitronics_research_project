...
def time_evolution_derivative_peierls(
    t: float,
    dt: float,
    ...
    A_t = -field_amplitude(t) * dt
    # Calculate the time-dependent Hamiltonian
    H = np.exp(1j * A_t) * H_hop
...
def rk4_step_peierls(
    ...
    """
    k1 = dt * time_evolution_derivative_peierls(t, dt, density_matrix, H_hop, field_amplitude, initial_density, decay_time)
    k2 = dt * time_evolution_derivative_peierls(t + 0.5 * dt, dt, density_matrix + 0.5 * k1, H_hop, field_amplitude, initial_density, decay_time)
    k3 = dt * time_evolution_derivative_peierls(t + 0.5 * dt, dt, density_matrix + 0.5 * k2, H_hop, field_amplitude, initial_density, decay_time)
    k4 = dt * time_evolution_derivative_peierls(t + dt, dt, density_matrix + k3, H_hop, field_amplitude, initial_density, decay_time)
...
def evolve_density_matrix_rk4_peierls(
    H_hop: np.ndarray,
    ...
    for step in trange(n_steps):
        ...
        t = step * dt
        D_t = rk4_step_peierls(t, D_t, H_hop, field_amplitude, dt, initial_density, decay_time)
